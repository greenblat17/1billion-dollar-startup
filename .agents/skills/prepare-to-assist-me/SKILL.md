...
- Notes are `wrong|||better` spliced into a quote; LLM must not treat spoken repair as errors.
- Two deploys: Redeploy cmp-server → `/opt/speaking-coach`; Redeploy ai-service → `/opt/ai-service` + Redis left running.
