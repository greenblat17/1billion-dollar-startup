...
Desktop UI: MCP `compose-hot-reload`. If the window is down, start `./gradlew :app:desktopApp:hotRun --auto` from `cmp/` in the background, then ask the user to respawn MCP: OpenCode → new session; Cursor → toggle `compose-hot-reload` in Settings → Tools & MCP (a new chat does not restart MCP). Shared UI is `cmp/app/shared/.../App.kt`. Cursor Gradle: `GRADLE_USER_HOME=$HOME/.gradle`. Viewport: `resize_window` **412×915** (Pixel 8) unless the task is explicitly desktop. After click/type, `get_logs` + `[Recomposition` is evidence (not only a screenshot). Plugin/annotation: skill `compose-stability-analyzer`. Do not add `org.jetbrains.compose.hot-reload` to the catalog as a plugin.
